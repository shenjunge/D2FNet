...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, num_class=4,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, int(num_patches+self.num_token), embed_dim))
        ...
        self.head1 = nn.Linear(self.num_features, 128)
        self.head_norm = nn.BatchNorm1d(128)
        self.head2 = nn.Linear(128, self.num_classes)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.head_norm(self.head1(x)))
        x = self.head2(x)
        return x
